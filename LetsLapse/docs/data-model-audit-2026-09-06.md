...
| 84 | `letslapse.deviceID` | `Shared/DeviceIdentity.swift` | String UUID · minted on first access | UD `DeviceIdentity.id` (once, never changed) | UD `DeviceIdentity.id`; `lapse audit --plist` prints it from a plist copy | **Added 2026-09-13 (Phase 1 W2).** The per-install id every sync-ready record names as `originDeviceID` / `modifiedBy` / `deletedBy`. The Mac's two preference domains (R7) hold two ids — two installs. |
