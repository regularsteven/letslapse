...
- **Stage 3 — switch transition (B).** *Core landed 2026-08-04 together with Stage 2* (Steven: skip SVGs for now, test on device): standard-world selection is a zoom ramp on the virtual device (no session transaction; `applyCaptureFormat` re-asserts zoom after format changes), and Stage 1's chip repaint fix (`ZoomChipButtonStyle`) landed with it. Still open from this stage: single-transaction + exposure seeding on the DNG-world discrete swaps, the freeze/dissolve cover for topology changes (DNG arm/disarm, cold start), continuous pinch (currently stepped, each step ramping), and re-measurement.
