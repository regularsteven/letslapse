...
- **Constituent switching is condition-based**: in the 16 Pro walk the tele stops reported `WideAngleCamera` as the backing camera — with `auto` switching the system engages the tele only when subject distance and light allow, and covers 5× from a wide-sensor crop otherwise (native Camera does the same). Not a mapping bug; the probe now logs the switching behaviour and suggests re-running aimed at a bright distant subject. Implication for the ramp: constituent choice belongs to the system, and the FOV the user sees at each stop is correct either way.
