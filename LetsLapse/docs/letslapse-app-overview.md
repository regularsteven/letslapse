...
- **Capture UI.** Persistent aspect-fit preview; speed chips with live output-length estimates per speed; a segment strip visualizing burst spans during recording; a target sheet (auto-stop with countdown ring); zoom as discrete lens chips (.5×/1×/3× where hardware exists); grid overlay; format pill (locked while capturing — video reads "2160p · 30 · Stab", interval reads resolution plus a JPEG/DNG token, DNG in amber); idle timer disabled during capture. The format sheet is mode-aware: Video gets frame rates, stabilization and speed-burst behavior; Interval gets the JPEG/DNG output picker instead (stills have no base frame rate), with a footer explaining DNG support or the fallback reason. An amber status line under the Interval dials states the output before recording ("Output: DNG · untouched originals, no blending", the ~1–2 s blend-cadence warning at sub-2 s spacings, or the DNG-unavailable reason). Last-used capture setup persists via `RecordingSettingsStore` (opt-out in Settings): the mode itself (Video/Interval) plus lens/resolution/rate/burst-rate/stabilization, interval spacing and frames-per-blend (upgrades fall back to the retired Live Blend mode's remembered spacing). The plain Record-now entry reopens in the remembered mode; effect cards still force theirs.
