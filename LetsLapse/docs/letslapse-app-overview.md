...
- **Interval** — stills on a timer (0.5 s–10 s presets, floor 0.5 s) with three dials — **EVERY** (spacing, plus `Auto`), **MODE** (Off · **Holy Grail** · **Scanner**, `IntervalCaptureMode` in `Shared/CaptureMode.swift`) and **BLEND** (frames per output image: fixed counts with "Off" = 1, plus the adaptive **Psycho**/**Safe** depths, §5.5) — and an output format (JPEG or DNG) chosen in the format sheet.

  MODE is what the dial between EVERY and BLEND selects: *which decision this shoot takes away from the timer.* It was called RAMP while Holy Grail was its only value, and was renamed when Scanner arrived — only one of the two is a ramp. **Holy Grail** (iOS/iPadOS only) hands shutter and ISO to `HolyGrailRampEngine` so one shoot runs daylight → night without flicker; it rides the Live Blend pipeline, so BLEND stays live and each output image is that window's frames averaged at that window's exposure. **Scanner** (`App/ScannerEngine.swift`, iOS/iPadOS only) hands the *shutter itself* to the scene: a preview tap differences consecutive frames down to a 64×48 luma grid, and a settled → disturbed → re-settled cycle fires a capture — put the object down, take your hand out, hear the click, repeat. Its frames are for photogrammetry solvers and compositing stacks rather than for a timelapse, which is why AE, AF and WB are locked for the whole run, why RAW is the default, why the device's own steadiness must agree with the scene's before any frame is taken, and why the capture screen offers a "Delete last" control, and why the finished set gets its own project screen (`App/ScannerProjectView.swift`) whose primary CTA is **Export frames** rather than "New blended clip". Phase 1 takes one frame per pose, so BLEND greys out under it.

  **Scanner Phase 2 — rectangles.** When something flat is in frame (a page, a document, a print), `App/RectangleDetector.swift` finds it on the same preview tap with `VNDetectRectanglesRequest` and the shoot changes in four ways. The quad is **traced over the live image** in the accent at 60%, corner markers and all. The settle test switches from the 64×48 frame difference to **corner stability** — all four corners still to within 2% of the frame across the whole settle window — which is a measurement of the object rather than of the frame: it catches a page creeping a millimetre a second and ignores a shadow crossing the desk. (Anything still *disturbs*: a hand crossing the frame or a page swapped in place must restart the window even though the corners never moved.) The corners are stamped into the per-frame sidecar at capture (`rectangle` in `frames.timestamps`, absent — not null — when nothing was in view), and `Kit/…/PerspectiveCorrector.swift` turns them into a rectified `frame-NNNNN-corrected.heic` beside the originals, at the stock the **PAPER** dial names (Auto · A4 · Letter · 4×6 · Square) — run from the finished set's own project screen (**Correct perspective**), never from the capture teardown, because the stock is the operator's call and a 36-pose set is seconds of GPU work. That screen (`App/ScannerProjectView.swift`, routed in place of the timelapse project body by `AppModel.isScannerProject(_:)`) is where a Scanner shoot now lands: a numbered frame grid over the rectified pages, **Export frames** as the primary action — a flat, sequentially-named folder, corrected page where there is one and the photograph where there isn't — and the blend path demoted to a secondary "View as timelapse". Projects carry `captureMode` for that routing; the sidecar's `rectangle` key is the fallback for sets registered before the field existed. **The overlay's own coordinates** are the other half of the geometry: Vision reports corners in the space of the image it was handed (the run's *capture* orientation, so the sidecar and the corrector need no conversion), while the viewfinder draws in the *interface* orientation — the same thing until the rotation lock is on or the device is flat over a copy stand — so the overlay un-turns the quad into camera space and asks `AVCaptureVideoPreviewLayer` where that lands, the same conversion tap-to-focus runs in reverse. Rectification takes the processed HEIC and never the DNG — resampling a Bayer mosaic mixes the sites that make it raw — and nothing overwrites either original. Selection between competing quads is **outermost first, confidence second**: a rectangle printed inside a photo is usually crisper than the photo's own border, so highest-confidence would rectify to the wrong thing. EVERY and MODE constrain each other: `Auto` is offered only when MODE isn't Off, Scanner requires it (the scene sets the spacing), and Holy Grail accepts either — on Auto it paces itself between a 3 s floor and a 15 s cap (`Kit/…/HolyGrailAutoInterval.swift`), on a fixed value the user has pinned the floor and the ramp manages exposure above it.

   The four combinations route to different engines behind one shutter (`startIntervalCapture()`): plain JPEG shoots use the photo-output dispatch timer (`frame-%05d.jpg`, Apple's full processed-still pipeline); JPEG blends run the video-tap Live Blend engine (§5.4); DNG shoots — blended or untouched — run the Bayer RAW pipeline (`frame-%05d.dng`, §6, iOS/iPadOS only; unsupported sources degrade per dial). All variants feed the same photo-stacking paths. (Live Blend was a mode of its own until July 2026; it merged into Interval as the BLEND dial — `CaptureMode(token:)` maps the retired wire/persistence value onto Interval.)
...
| Vignette | −1 … +1 (**positive darkens**), plus a midpoint 0 … 1 (neutral 0.5) | Metal `applyVignette` `{strength, midpoint}` on the still path; `CIVignette` (max intensity 2.0, radius 1.5 at the neutral midpoint) on the display-referred chain — signed since 2026-09-12, see *The editor* below |
...
That table is the original 2026-07 field set, kept because the decode-field-by-field rule still holds for every one of them. The manual grade has since grown into the Lightroom-shaped panel the editor's groups list below: exposure, contrast, highlights, whites, shadows, blacks; an *owned* white as `whiteMired` / `whiteTint` (an absolute, keyframed declaration of the white the frame is rendered at — a correction, not a look — beside the legacy `temperature` / `tint` offsets, which no control shows any more); vibrance, saturation and the eight-band HSL mixer; texture, clarity, dehaze, the signed vignette and its midpoint; sharpen with its edge masking, noise reduction with its detail and colour-noise controls; a `.cube` LUT; and two **geometry** corrections — `rotationDegrees` (the ±10° straighten of 2026-09-02, `RotationSlider` over the Kit's `FrameRotation`) and `crop` — which the colour engine never sees and presets never carry. Stills render through the linear-light Metal tone engine (`Kit/Sources/LetsLapseKit/Grading/GradeEngine.swift`, recipe in `GradeRecipe.swift`); the Core Image chain survives verbatim as the Kit's `DisplayGrade`, which is what a masked grade runs over a finished picture and what the video composition still grades through.
...
- **`PhotoViewerView`** (`App/PhotoViewerView.swift`) is the fullscreen grading editor for photo and interval captures, and **`VideoEditorView`** (`App/VideoEditorView.swift`) its twin over a movie's own clock. Both are built around the same `PhotoAdjustmentsPanel` and the same three layouts (*The editor*, below); renders are debounced at ~100 ms and keyed on a bumped token so a burst of pad ticks collapses into one render (the video editor waits longer, since swapping a composition restarts the player's pipeline). On iOS the viewer is a cover with its own back disc; on macOS it *is* a window (`PhotoEditorWindowRequest`), because Mac sheets can't be resized.
- **`PhotoAdjustmentsPanel`** (`App/PhotoAdjustmentsPanel.swift`) is the one definition of the controls — one `EditorGroup` at a time, dressed for whichever layout its owner is in — used by both editors and by the masked-grade card, so the surfaces can't drift.
...
#### The editor — six groups, pads, and one panel at a time

Rebuilt on 2026-09-13 from the signed-off *LetsLapse Editor slider organization* handoff (boards 2a · 5a · 3b for the three layouts, 6a–6c for where the timeline goes, 6d/6e for the Settings toggle and sliders-only mode); the lock-in and its decisions are recorded in the dated notes at the top of each `docs/design/*/INDEX.md`. What changed is the *organisation* of the controls, not the grade: `PhotoAdjustments`, `GradeTimeline` and the render paths are the same, with two additions — a crop and a signed vignette — described below.

- **Five groups plus Presets, opened one at a time.** `App/EditorControls/EditorGroup.swift` is the vocabulary. `EditorGroup` (`presets · light · color · effects · detail · crop`) is the unit the six main buttons, the header dot, the per-group Reset, the ✓/✕ snapshot and the `LL_SECTIONS` hook all speak in; `EditorTool` is the row of chips inside a group — Light holds `expCon · highWhites · shadBlacks`, Color `whiteBalance · vibSat · mixer`, Effects `texClar · vignette · dehaze`, Detail `sharpen · noise`; Presets shows a thumbnail strip and Crop its aspect chips plus the Angle slider, so neither has tools. Each tool declares its pad (`PadSpec`: the Y field, the X field, four margin words, a gradient background that previews what the axes do) and its fields; a group's `fields` is the union — plus `colorNoise` under Detail, which no chip shows but a Lightroom import can still write, so the group's dot has to light for it. White Balance and the Color Mixer became tools inside Color, Rotation became the Angle slider inside Crop, and the old seven-section panel is gone. The chip icons are drawn in SwiftUI (`EditorToolIcon`, parameterised by ink colour); the main buttons keep SF Symbols.
- **XY pads** (`App/EditorControls/XYPad.swift`) are the redesign's control: one knob, two fields, Y up is always "more" of the dominant value — Exposure over Contrast, Highlights over Whites, Shadows over Blacks, Temp (warm up) over Tint (green → magenta), Saturation over Vibrance, Clarity over Texture, Vignette (lighten up) over its midpoint, Sharpen over its masking, Noise over colour noise with the Detail slider underneath; the Mixer's pad binds the selected band's hue and luminance with a band-saturation slider under it. The pad speaks only in normalized coordinates and the panel does the field arithmetic — range, presentation sign, neutral — in one place beside the slider rows it replaces. A drag writes BOTH fields through the one `Binding<PhotoAdjustments>` in one assignment per tick, because with a timeline in play each assignment is one keyframe write and two writes would be two; `onFieldEditing` brackets the drag for both fields (the viewer's 1:1 loupe under Sharpen and Noise, persist on release); a double-tap resets both through `onResetField`; a bare tap writes nothing (`dragSlack`, 3 pt). Readouts sit under the pad in the app's existing formats, in the accent — with a 7 pt diamond beside them and on the tool's chip — whenever the field travels over the shoot (treatment 1e-A). Settings › Advanced › Layout › **Use Pads in Editor** (`LayoutSettings.editorPadsKey`, on by default) turns every pad back into its two sliders, Y field first, with grouping, presets and the timeline unchanged; `LL_PADS=off` does the same for a screenshot without writing the setting. Temp and Tint keep their gradient tracks in sliders mode (`GradientTrackSlider`); the other rows are the native `Slider` for now (`docs/TODO.md`).
- **✓ / ✕ are the editors', not the panel's.** The panel owns no render state and no snapshot: it draws one group under one of three dressings (`EditorPanelLayout.phone · floating · rail`) in one of two styles (`XYPadStyle.dark` on the iOS editors, `.light` on the Mac) and reports `onCommit` / `onCancel`. Each editor holds the open group, the per-group chip selection (owner-held so it survives the phone sheet being torn down between opens), and an `EditorSnapshot` — preset, adjustments, the whole `GradeTimeline`, the `PresetState` and the shoot's white-balance source — taken in `openPanel(for:)` when a main button is tapped. ✓ (`commitPanel`) drops the snapshot, keeps the values and persists as a finished gesture; ✕ (`cancelPanel`) writes every part of it back — with a timeline that means restoring the keyframes, since the edit landed there rather than in `adjustments` — and carries the text layers back through the level change (`carryOverlays`) rather than restoring the overlay document, which the Text and Masks tabs may have moved meanwhile. Tapping another main button commits the open group and snapshots afresh; tapping the open one again does nothing, so a stray tap can never throw an edit away; switching rail tabs commits; opening Crop resets the zoom to fitted on every layout, so the frame and its handles are all on screen. On the Mac rail the Masks card lists its grades as compact rows carrying the tool icons of the fields each grade moves (`App/Overlay/MasksCard.swift`, "Manage ›" switches to the Masks tab); its expanded editor still draws the pre-redesign sections, and the touch layouts show the card only as a stopgap while a grade is expanded — both in `docs/TODO.md`.
- **Three layouts, chosen by size and shape** (`EditorLayout` and `editorLayout(for:)` in both editors): the Mac is always the **rail**; a container at least 900 pt wide and wider than tall is the iPad's **floating** layout; anything else at least 500 pt wide — a landscape iPhone, a wide split, and iPad portrait (settled 2026-09-13 over the spec's "iPad portrait follows the phone": wide enough for a rail, too tall for a sheet to reach the picture) — is the rail drawn dark; the rest — every iPhone portrait, Slide Over — is the **phone** layout. No idiom check anywhere. On the touch layouts the Editor / Text / Masks tab bar becomes a translucent pill top-right over the picture (`EditorTabPill`), the Mac keeps `RailTabBar` in its rail, and the INTERVAL / VIDEO chip is `EditorMarqueeBadge` (kind · duration · frame count). *Phone (2a/6a):* the picture fills the safe area and the foot stacks the timeline card and then EITHER the six main buttons (`EditorGroupBar`, style `.phone`) OR the open group's sheet — the sheet needs the buttons' room; the foot is measured so the picture's corner chrome sits above it, and Crop fits the picture into the room above the foot so the frame's handles can be reached. *Floating (5a/6b):* the picture anchored top-left at the size its aspect gives it, back disc and INTERVAL chip top-left, the compact timeline strip in a 66 pt capsule along the foot beside the buttons pill bottom-right, and the open group as a 400 pt card floating above the buttons, draggable by its header (its offset kept for the session across groups); Presets fills the height instead, and Crop centres the picture inside a margin (48 pt sides, chrome-clear top, foot-clear bottom) so every handle is draggable. *Rail (3b/6c):* the media pane with the INTERVAL badge bottom-left (the video editor's VIDEO badge sits top-left, clear of AVKit's hover transport), the compact timeline in a **54 pt row under the pane** — it belongs to the media, so it takes the pane's width — and a rail (`railWidth(in:)`: 330 pt on the Mac, `min(340, width × 0.42)` on iOS) holding the tab pill, the six-button card, the open group's card (Revert / Done), the Masks card and "Reset adjustments". `GradeTimelineView`'s metrics are unchanged everywhere; only its container moved. The video editor reuses all of it over the movie's clock, with no Auto button and no Masks card.
- **Crop** is the second geometry field: `PhotoAdjustments.crop: FrameCrop?` (`Kit/Sources/LetsLapseKit/Grading/FrameCrop.swift`) — a rectangle of the LEVELLED frame's unit square, origin top-left, plus the aspect it was drawn under (`original · square · fourFive · sixteenNine · nineSixteen · custom`). Normalized so one crop serves every render size; in the levelled frame because that is the picture the frame was drawn over. It is treated like the rotation — a correction of one photograph, not a look — so it stays out of presets and out of the Original/Edited verdict (`withoutGeometry`), and the colour engine never sees it. Unlike the rotation it is **static, by decision**: the timeline never blends it, and `GradeTimeline.carryCrop` stamps one crop onto the baseline and every keyframe (creating none), so `PhotoGrade.crop`, `crop(at:)` and `hasCrop` are one answer at any playhead position; the editors' `editedAdjustments` setters lift the crop out of a write and carry it separately. A whole frame under `.original` normalises to nil (the `hsl` pattern), so an uncropped project keeps its payload byte for byte; a whole frame under any other aspect is kept, so the chip round-trips. In the editor the picture is **never resized by the crop**: `App/EditorControls/CropFrameOverlay.swift` draws it over the full levelled picture — a 55 % scrim with thirds, corner handles and the gestures while the Crop panel is open (drag the body, drag a corner about the opposite one, pinch about the centre; locked ratios stay locked; every gesture freezes its base and applies absolute deltas), an 85 % scrim and a 1 pt frame when closed — so zoom, 1:1, text layers and masks keep their coordinate spaces. The render paths cut it on the way out: `PhotoGrader.render(cropped:)` defaults to true and the editor passes false; the stills blend bakes uncropped frames (`OverlayExportBake` composites in the full levelled frame) and the tail pass cuts the clip (`VideoCanvasCropper.croppedCopy(of:crop:…)` — crop first, then any chosen canvas fitted inside it); `stackPhotos` reads `grade.crop`; the video path cuts inside `VideoGrader.composition(cropped:)`, whose default is FALSE because a composition's usual home is the editor's player — the player shows the whole movie and the Crop panel's hint says "shown on export" (`cropNote`) — while `bakedCopy` and the motion preview pass true and the composition's `renderSize` follows the crop. Two paths set it aside and say so in the render summary — a punch-in reframe (" · crop not applied (punch-in reframe)") and the time-slice poster fast path — and, with the Adjust screen's missing crop twin, are `docs/TODO.md` entries.
- **The vignette is signed, with a midpoint.** `vignetteIntensity` runs −1…+1 with **positive = darken** — the convention every stored project and preset already meant, so nothing migrated — and negative lightens. The panel shows it Lightroom-style, up = lighten = "+", by negating at the control the way the Temp control negates mired. `vignetteMidpoint` (0…1, neutral 0.5, encoded only off the middle) is where the falloff starts. In the Metal engine (`applyVignette` in `GradeKernelSource.swift`, `{strength, midpoint}`) the smoothstep starts at 0.6 × midpoint — 0.3 at the default, so every existing render is bit-identical — darkening multiplies as it always did and lightening moves each channel toward 1.0 by the same fraction; the display-referred chain (`DisplayGrade`) maps the midpoint onto `CIVignette`'s radius, which runs the opposite way (1.5 at neutral, 2.0 at 0, 0.5 at 1 — loose parity by nature). The Lightroom import carries `PostCropVignetteAmount` (or the lens amount) sign-flipped, with its midpoint. The still and blend paths centre the vignette on the whole frame and the video path on the crop — a `docs/TODO.md` entry.
- **WB Auto** is a button, not a mode. Kit `AutoWhiteBalance.estimate` (`Grading/AutoWhiteBalance.swift`) takes the frame under the playhead AS RENDERED (the owner renders it at ≤ 256 px; the estimator averages a 128 px copy, near-black and clipped pixels left out) and returns the grey-world white to declare INSTEAD: mean linear RGB → XYZ → xy → CCT by McCamy's fit, plus the distance off the Planckian locus turned into the converter's ±150 tint, with the estimator's own reading of D65 subtracted so a neutral grey lands exactly on the current white. It is wrong on purpose whenever the scene is not grey on average — a sunset is orange because the light is — which is why it is pressed and judged rather than defaulted, and why the panel writes the result through `ownWhite(kelvin:tint:)` like a menu pick: keyframed, undoable, and seeding every unowned moment's white as any owned white does. The menu reads *Auto* for exactly as long as the white under the playhead IS the estimate (a value compared against, not a flag cleared, so a scrub, a ✕ or a preset drops the name by itself); the video editor passes `autoWhite: nil`, since a movie has no raw white to declare, and the button is hidden.
- **Presets** shows live tiles: `PresetThumbnailCache` (`App/EditorControls/PresetThumbnails.swift`, the cache itself in `App/ManagePresetsView.swift`, bounded at 240 images) renders the frame under the playhead at 160 px off-main for the built-ins of `PhotoPreset.strip` and every saved `CustomPreset`. A tile renders the preset ALONE (settled 2026-09-13): a tap replaces the edits through the existing `request(_:)` path, which confirms first, so the tile shows what the tap produces. A horizontal strip on the phone, a four-column grid filling the height on the iPad, a three-column grid in the Mac card; the selected tile carries the accent ring.
- **Hooks.** `LL_SECTIONS=<group>[:<tool>[:<band>]]` opens a group's panel with a chip up — the legacy `wb`, `mixer[:axis]`, `rotation` and `all` still answer — on every layout and in the video editor too; `LL_PADS=off` forces the sliders; `LL_KEYFRAMES=sunset` stages three moments that OWN their whites (6100 / 7600 / 5400 K) so the boards' keyframed Temp reproduces. The full grammar is the hooks paragraph of `docs/design/README.md`.
...
- **`GalleryPreviewPanel`** (`App/GalleryPreviewPanel.swift`) is what a single tap raises — the iPhone's sheet, the iPad's and the Mac's fixed 300 pt pane. Top to bottom: thumbnail, title, Open / Edit / Text / Shapes / New clip, then **TAGS** (the shared `TagField` over the project's keywords — tags *are* keywords, one list in the manifest's `sceneTags` and the record's `dc:subject`, always the whole project's), a collapsible **PRESETS** row (the editor's own preset tiles through `EditorPresetsContext.tiles`, this project's hero rendered through each preset, so a look goes on a project without opening the editor — applied through `AppModel.applyPreset` / `applyCustomPreset`, which keep the project's rotation, crop and owned white, behind the shared `PresetApplyRequest` confirmation when the project is Edited), then the scope switch an interval project gets, **INFO** and **METADATA** (`App/MetadataPanelSections.swift`), the meta rows, the footer and the blended-clips list. The order is Steven's ruling of 2026-09-13: tags are the metadata a person actually manages and sit above everything the files said. Mirrors: `docs/design/macOS/gallery*.svg`, `iOS/gallery.preview*.svg`; hooks `LL_SELECT`, `LL_PANEL`.
- **Multi-selection and batch mode** (`App/GallerySelection.swift`, 2026-09-13): one tile selected is the preview panel's; more than one is batch mode. ⌘-click toggles, ⇧-click selects the run from the anchor in the grid's order, ⌘A selects the filtered grid (a local key monitor scoped to the Gallery's window that steps aside for a focused text field); the tile menu's **Select Multiple** (every tile wears a circle, taps toggle) and **Select All** do it on every platform. The header becomes the selection row — Done · "N selected" · Select All / Deselect All, plus the Edit glyph on the phone — and the pane (the phone's sheet) becomes **`GalleryBatchPanel`**: tags as the union (solid on all, mixed "n/N" on some; add to all / remove from all), the preset tiles previewed on the first selected and applied to all (the confirmation counts the Edited ones), and the IPTC record with "Mixed" placeholders, a commit writing all and a revert reverting all. Hooks: `LL_SELECT=all|multiple|<uuid>,<uuid>`. Keyboard (Mac): the arrows move the selection to the neighbouring tile — up and down keep the column, in the timeline's day groups five across — ⇧-arrow grows the run, ⌘D selects none, and 0–5 rate every selected project (0 clears); the circle on a tile toggles it without touching the rest, and a click in a gutter or the margin selects none. Tiles and the panel's hero render through the project's grade (`ProjectThumbnailCache.thumbnail(for:kind:grade:)`, memory-cached by file + grade token), so a preset shows in the grid the moment it lands. Mirrors: `macOS/gallery.menu.svg`, `gallery.batch.svg`; `iOS/gallery.menu.portrait.svg`, `gallery.selecting.portrait.svg`, `gallery.batch.portrait.svg`, `gallery.batch-sheet.portrait.svg`; the mixed record is `components/metadata-fields.mixed.*`. Not there, by scope: batch delete and batch New clip — the tile menu's Open / Edit / Delete act on the one tile.
...
- **Advanced** — **Blend learning** (this device's learned Psycho profiles per pipeline × interval × thermal bucket, each with its Safe count, run count and best/worst bounds, plus a confirmed destructive reset; §5.5), **Layout** (`App/LayoutSettings.swift`, plain `UserDefaults` keys read by view bodies through `@AppStorage`: **Enable Scans menu**, **Display count in Projects**, and — since 2026-09-13 — **Use Pads in Editor**, on by default, whose off state renders every editor pad as its two sliders on iPhone, iPad and Mac alike with grouping, presets and the timeline unchanged; §4.5 *The editor*), **Performance** (CPU worker budget, concurrent blend batches — used by the macOS job runner) and **Diagnostics** (latest job folder path and processing log).
...
├── Projects/
│   ├── library.json                 # since M1 (2026-09-14) a GENERATED compatibility export of the documents below ("generated": true + record counts in its tail); since M3 regenerated at launch when stale and at quit, not per persist; never loaded
│   ├── library.json.pre-switch-*    # the last manifest a pre-switch build wrote, kept by the first launch that flipped (one file; delete once checked)
...
│   └── <captureUUID>/
│       ├── project.json             # THE project's record: {formatVersion 2, capture, blends} — what the app loads (M1), writes on every persist, and what a .lapse and a transfer carry
...
**The data-model programme (2026-09, `docs/data-model-*.md`)** made the per-project files the truth: since M1 (2026-09-14) the app **loads every `Projects/<id>/project.json`** (live and `.trash`, one strict decode each, so one bad file costs one project) plus `Collections/collections.json`, and `library.json` is a generated compatibility export it writes after every persist (`"generated": true`) for older builds, the transfer catalogue and `lapse audit`. `lapse audit <root> --rebuild-index` reconstructs the manifest from the documents and diffs it against the export — the check that must read IDENTICAL after any change to the persist path. At launch the app (1) takes or respects `Projects/.lock`, (2) reads the documents (`LibraryDocumentLoader`: a document under `Projects/` is live whatever its tombstone says, one under `.trash/` is deleted; a document that will not decode is reported and left alone; a library with no documents at all bootstraps from its manifest once), keeping a `library.json.pre-switch-<stamp>` copy the first time it meets a manifest without the marker, (3) adopts any UUID folder under `Projects/` that has no record (from the manifest's record if it has one, else as a "Recovered" project from its media), (4) sweeps the trash, (5) brings every `project.json` and `collections.json` up to the loaded state and the SQLite index up to the files (a fresh or discarded index is rebuilt whole), (6) regenerates the export if it was missing, unreadable, pre-switch or no longer lists what the documents say. Every persist goes through one version-gated `LibraryPersister`, which writes the documents that changed and their index rows first, then the export. **Since M3 (2026-09-14) nothing holds the whole library in memory:** `ProjectStore` reads a project's document when a screen asks (`capture(id:)`, an LRU of 512), every change writes that one document and its index row through the per-project persister, the launch is a walk of the folders against the index on the persister's queue (`LibraryReconciler`: a stat each, a read only for a document the index does not know as it is on disk), and `library.json` is regenerated from the documents at launch when its tail says it is stale and at quit / background — not per persist. **Since M2 (2026-09-14) the lists read the index:** the Projects list, the Gallery and the collections clip picker take their rows — order, kind filter, tag chips, counts and the FTS5 prefix-per-word search — from `LibraryIndex` through one `ProjectListQuery` (`App/AppModel+Lists.swift`), each card from its record by id (`capture(id:)`, `blend(id:)`), and a single-project change goes through `updateCapture(_:edited:persist:_:)` (the seed of Phase 5's `apply(change)`); the index row carries the lists' category (photo / interval / video / scan by the app's rules, `Kit/…/ProjectCategory.swift`), the Edit date and the SHAPES counts. `lapse index <root>` reads or rebuilds the index headless; `lapse import-lightroom` migrates a Lightroom catalogue (read-only on the catalogue) into folders the app adopts.
...
- **The photo editor is a window, not a sheet.** Mac sheets are never user-resizable, so `PhotoViewerView` is hosted by its own `WindowGroup(for: PhotoEditorWindowRequest.self)` — one window per photo, reopening the same photo fronts its window, 1000×700 default, 720×480 floor (the rail is fixed at 330 pt since the 2026-09-13 editor redesign — board 3b; it was 340 — leaving the image pane a workable ~390 pt at the smallest, with the grade timeline in a 54 pt row under the pane rather than in the rail).
...
- **Components.** `LLPrimaryButtonStyle` (accent fill, 50 pt min height, radius 14), `LLSecondaryButtonStyle` (card fill, tinted label), `.llCard()` (radius 16–18, soft shadow), `LLRow` (settings-style rows), `LLSectionHeader`, `MediaBadge` (translucent capsule over imagery), `FloatingTabBar` (material pill; re-tap pops to root), `FlowHeader`/`FlowChromeButton` for the overlay flow, `CaptureFilterBar` and `CaptureAssetGrid` for the browsing surfaces, `PhotoAdjustmentsPanel` with the `App/EditorControls/` set for grading (`EditorGroupBar`, `EditorTabPill`, `EditorToolChips`, `EditorToolIcon`, `XYPad`, `GradientTrackSlider`, `PresetTile`, `CropFrameOverlay`, `EditorMarqueeBadge` — the 2026-09-13 editor redesign's vocabulary, §4.5 *The editor*), and shared async thumbnails.
...
**App:** open `LetsLapse/LetsLapse.xcodeproj`. Two targets: `LetsLapse` (iPhone/iPad/Mac) and `LetsLapse Watch App` (embedded into iOS builds automatically). Steven's team and bundle identifiers are checked in; anyone else picks their own under Signing & Capabilities. The step-by-step for a fresh Mac or a newcomer — Xcode components, package resolution, signing, the errors decoded — is `docs/building.md`. The app target links the MLX engine's LM library from a **committed** patched copy of `mlx-swift-lm` at `tools/mlx-vlm-spike/vendor/` (regenerated by `vendor/refresh.sh`, never edited by hand), so a checkout resolves with no extra step.
...
| `LL_VIEWER` = `1` \| `expanded` | open the newest photo/interval project's grading viewer on its hero frame (`expanded` only still means something on the Text / Frames / Masks pages — the Editor page gives the picture the whole screen) |
| `LL_EDITOR` = `latest` \| `video` \| *capture-uuid* | open the grading editor directly — `latest` the photo editor on the largest interval shoot, `video` the video editor on the newest movie, a UUID a specific project (on iOS `video` stops at the project detail; tap Edit) |
| `LL_SECTIONS` = `presets` \| `light` \| `color` \| `effects` \| `detail` \| `crop` [`:`*tool*[`:`*band*]] | open that group's panel with a chip up (`expcon`, `highwhites`, `shadblacks`, `wb`, `vibsat`, `mixer`, `texclar`, `vignette`, `dehaze`, `sharpen`, `noise`; a mixer band by its Lightroom name); the legacy `all`, `wb`, `mixer[:axis]`, `rotation` still answer. Every layout, both editors |
| `LL_PADS` = `off` | render the editor's pads as sliders without touching the Use Pads in Editor setting |
| `LL_KEYFRAMES` = `sunset` \| `empty` | stage the grade timeline's design scenario (three moments at 10 / 52 / 86 %, each owning its white) or the first-run strip; pair with `LL_VIEWER=1` or `LL_EDITOR` |
...
| Colour grading | `App/PhotoPreset.swift`, `App/PhotoAdjustments.swift`, `App/VideoGrader.swift`, `App/CustomPreset.swift`, `App/PhotoViewerView.swift`, `App/VideoEditorView.swift`, `App/GradeTimeline.swift`; the engine in `Kit/Sources/LetsLapseKit/Grading/` (`GradeEngine`, `GradeRecipe`, `DisplayGrade`, `FrameCrop`, `AutoWhiteBalance`) |
| The editor's controls | `App/EditorControls/EditorGroup.swift` (groups, tools, pad specs — read this first), `App/PhotoAdjustmentsPanel.swift`, then `XYPad.swift`, `CropFrameOverlay.swift`, `EditorGroupBar.swift`, `PresetThumbnails.swift` in the same folder |
