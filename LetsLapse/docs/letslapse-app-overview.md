...
- **Two capture modes.**
  ...
  - A third `CaptureMode` case, **Live Blend**, blends several camera frames into each timelapse image during capture, on every platform (§5.4; DNG output on iOS/iPadOS, §6).
- **Capture UI.** Persistent aspect-fit preview; speed chips with live output-length estimates per speed; a segment strip visualizing burst spans during recording; a target sheet (auto-stop with countdown ring); zoom as discrete lens chips (.5×/1×/3× where hardware exists); grid overlay; format pill (locked while recording); idle timer disabled during capture. Last-used capture setup persists via `RecordingSettingsStore` (opt-out in Settings): the mode itself (Video/Interval/Live Blend) plus lens/resolution/rate/burst-rate/stabilization, per-mode interval spacing (Interval and Live Blend remember separate seconds) and Live Blend's frames-per-blend. The plain Record-now entry reopens in the remembered mode; effect cards still force theirs.
...
- **Live Blend DNG is iOS/iPadOS-only by SDK decree** (§6.1) — the Bayer RAW capture API is unavailable on macOS, so Macs always run the JPEG video-tap path. The on-screen diagnostics readout is expected to disappear if the feature graduates.
