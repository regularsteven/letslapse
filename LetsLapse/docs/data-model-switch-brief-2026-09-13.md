...
**Date:** 2026-09-13 · **Status:** M1 landed 2026-09-14 (538dbb1 the Kit's `LibraryExportFormat`, 77116f1 the app loads the documents; report `data-model-audit-reports/scratch-m1-switch-2026-09-14.txt`); M2 waits on the §9 decisions · **Branch:** `ios-app` at 6ca7c64 when written · **Builds on:** Phases 1–4 and the Lightroom tool, all landed 2026-09-13 (commits 7fd0a0a → 6ca7c64, one per work item)
...
**Landed 2026-09-14** (`LibraryDocumentLoader`, `LibraryExportFormat`; report in `data-model-audit-reports/`). What the build of it settled, beyond the paragraph above: the first launch that meets a manifest without the `generated` marker keeps it as `library.json.pre-switch-<stamp>` (the acceptance diff's other half, and the only copy of a record with no folder); a document under `Projects/` is live whatever its tombstone says and one under `.trash/` is deleted (Phase 4's hand-restore rule, now for every folder); a document that will not decode is reported, left byte-for-byte alone and kept out of the folder reconciliation; a live folder with no document takes its record from the manifest once; a library with no documents at all bootstraps from its manifest and flips on that launch; the export is also regenerated when its id sets no longer match the documents; the loader seeds the document writer, so an adopting launch's persist writes one document (28 ms) rather than every document and index row (8 s). Two things the diff shows that are not defects: the manifest's µs are gone at the millisecond, and two twin projects with byte-identical `createdAt` swap places once (folder order instead of manifest order). Scratch-root numbers on the Mac: 366 documents in 0.09–0.16 s. **Owed:** the real Mac library's first M1 launch and its read-only audit after (this session could not run `lapse` against the volume); the iPhone 16 Pro launch time at 518 (a device install — ask first).
