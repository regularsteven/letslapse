...
**code-first by decision, server ask included** · **stage A LANDED 2026-09-16** (plan §9) · **Steven's first test the same evening mixed two libraries in one account (plan §10) → decisions L13–L20: session per Mac not per library, one Sign In / Sign Out, switching never disconnects, one-screen switch/create, names required, connect never automatic, the nest retired (recommended), preview-only projects show their poster; **stage A′ LANDED the same evening (plan §12: L13–L20 — session per Mac, one Sign In / Sign Out, Disconnect in the row menu, one-screen switch/create, names with the unnamed state and the connect question's name field, merge refused until the server scopes libraries, no automatic connect question, nest retired, poster in the hero + disabled editor buttons + Finder reveals the folder); the server's stage B was BUILT the same night (picplace `5786a9e`, `docs/letslapse-libraries.md`); **stage C1 LANDED 2026-09-16 night (plan §14: the connect sheet — new / take over unfiled / link — with the numbers, scoped passes with departure notices, `library` on creates only, rename sync, Add Library from PicPlace…, verified end to end on picplace.test with a throwaway account); C2 (iOS active library) owed; the mixed volume library was cleaned up with Steven's go (plan §13); **Steven's phone walk 2026-09-17 (plan §17): the connect sheet counted pulled previews, disconnect kept them, L11 (iOS library = a filter) REVERSED → L21–L25: iOS gets the Mac's folder model (`Libraries/<id>/`, one open, switch by replacing the model), disconnect evicts previews (never a tombstone), counts originals-only; **step 0 LANDED 2026-09-17 afternoon (plan §17.7: counts originals-only, disconnect keeps previews with the numbers, eviction at a connect elsewhere, the sheet leads with the former library, a preview's failed push retried, `LL_PICPLACE_OFFER`); **C2a LANDED the same evening (plan §17.8: iOS `Libraries/<id>/`, `storage.activeLibrary`, the folder registry, the marker-guarded one-time move, logs at the container); **C2b LANDED the same night (plan §17.9: `ModelHost` switch in-process, stand-down, stores re-rooted, the retired model made light — UIKit keeps it); **C2c LANDED the same night (plan §17.10: Settings ▸ Libraries on the phone — Switch, Add from PicPlace, New Library on PicPlace, Remove with its originals guard, Rename; `LL_LIBRARY` hook); **C2d LANDED 2026-09-18 (plan §17.11: the Projects header's library menu on the phone; every mirror drawn, INDEX rows ✅ on iOS/iPadOS/macOS; the staging hook's demo values made obviously staged) — the programme's stages A → C2 are closed; stage D (move without originals) remains this entry's open item; **2026-09-18 the card's numbers simplified (plan §17.12): *518 of 879 projects · bytes* with *originals for N · N not on PicPlace yet* under it, other libraries never named, previews filed elsewhere removed by the check** · large · seams:
