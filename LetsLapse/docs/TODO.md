...
## Run-time cluster toggles: Dim and Info — code mirror of the 2026-09-04 design (third pass)

Design first: `docs/design/components/shutter-cluster.running*.svg` (Dim in slot 1, Info in
slot 2), `iOS/capture-interval.running*.portrait.svg`, `…holygrail-running`, `…ladder-running`,
`iOS/capture-video.recording*.svg`, `macOS/capture-interval.ladder.running.svg`; contract in
`components/README.md`. The job:

1. **Dim toggle** (`clusterSlot(1)` while `isCapturing`, iOS only — `ShootScreenDimmer` is a
   no-op on the Mac, leave the slot empty there): a per-run `dimEngaged` seeded from the
   Settings default (`ShootScreenDimmer.defaultsKey`, on by default). On → `setEngaged(true)`
   floors the screen at once; the cover's tap → `wake(for: 30)` (today 8 s) and it re-floors by
   itself; pressing the toggle off during the wake cancels the re-floor. The existing Watch /
   remote flips of the setting keep working and should move the toggle too. Amber `moon.fill`
   while engaged, white `moon` off.
2. **Info toggle** (`clusterSlot(2)`): per-run `@State showRunInfo = false`. Interval: the
   running branch of `intervalStatusRow` / `landscapeIntervalRow` becomes [info panel if on] +
   amber exposure line + ±EV slider (portrait's `exposurePanel` already draws the slider under
   the ramp — reuse; landscape needs the slider in the corner row). The amber line: shutter ·
   ISO · scene EV from `holyGrailState` under Dynamic/Ladder; a **Basic** run has no ramp state,
   so publish the live pair off the device (`exposureDuration` / `iso` KVO, `previewSceneEV`
   exists) — and give Basic runs a bias slider through `setExposureTargetBias`. The ramp's
   status ('ISO ramping' amber, 'past the sensor's limit' red) rides the end of the amber line.
   Drop `intervalRunningPills` and `holyGrailReadout` from the running branch (the BurstStatus
   pill stays for Photo bursts); the diagnostics panel gains the thermal word on its last row.
   Video: `showRunInfo` gates `speedMarquee` in both `portraitControls` and
   `landscapeRecordingReadout`; the segment strip always shows.
3. **Decision to confirm with Steven**: the spec drops the output count and elapsed clock from
   every running state (they were the pills row). If they should survive, the cheapest seat is
   a fourth row on the Info panel or a small pill beside the amber line.
4. Hooks: `LL_RECORDING=1` / `LL_LADDER=running` / `LL_HOLYGRAIL=running` should accept
   `LL_RUNINFO=1` to stage Info on for the two new mirrors. Verify on the sim (portrait +
   landscape) and the Mac, then flip the INDEX rows.
