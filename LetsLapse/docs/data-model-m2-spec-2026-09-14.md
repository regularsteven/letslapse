...
**Date:** 2026-09-14 · **Status:** W1–W3 landed 2026-09-14 (f7eb131 the Kit's schema 2, 298ff6c the list hooks, 00a7513 the lists over the index, 3fc7a85 the accessor + funnel); W4's record in `data-model-audit-reports/scratch-m2-lists-2026-09-14.txt`; mirrors updated desc-only, Steven's look owed · **Implements:** [the switch brief](data-model-switch-brief-2026-09-13.md) §4 M2 with its §9 decisions · **Branch:** `claude/ios-app-data-model-a4ffdd` on `ios-app` a7f9810, after M1 (538dbb1 → 2fc20c0)
...
2. **Search.** Prefix per word replaces substring: `brid` still finds *bridge*; `idge` no longer does. Everything else a word matched before — the name, a tag's raw value or its label, an element — it still matches, plus asset titles, captions, keywords, creator, place and camera. The cases to record before and after: `night water` · `Sky & weather` / `weather` · a hand-typed tag · `brid` · an element word · `idge` (the one that changes). **Found while measuring (2026-09-14):** two more, both from the search now reading the *record* rather than the *rendering* — an unnamed project's dated display title ("Photo 13. 9. at 13:49") is not a field, so a typed date fragment no longer finds it; and "photo" now also finds unnamed interval shoots through their `originalName` ("1057 photos"), which the display title never showed. A word with no letter or digit ("&") is dropped rather than sent as a term.
