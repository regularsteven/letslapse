...
| Adjust (video source) | [adjust.portrait.svg](adjust.portrait.svg) | `App/AdjustView.swift` + `App/WarpTimelineView.swift` (`sourceCard`, warp card — thumbnail · bar · seams · selection, `chipsRow`, `estimateCard`) — the 3a warp timeline | ✅ |
...
| Adjust (video) · stretch menu open | — | `App/WarpTimelineView.swift` (`stretchMenu` — Remove stretch · Split here · Reset speed, from the ⋯ button) | 🟡 |
| Adjust (video) · nominating (drag overlay) | — | `App/WarpTimelineView.swift` (dashed amber overlay while carving a 1× stretch) | 🟡 |
