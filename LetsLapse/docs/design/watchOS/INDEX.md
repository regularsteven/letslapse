...
Last full sync: 2026-08-11, working tree of `ios-app` — **timed-burst chips 1s/2s/4s → 1s/4s/8s** (ramp review: seam eases are funded by burst footage, and under ~4s at 100fps a ramped seam eats the whole burst bridging its own cuts; 1s stays as the just-a-beat accent). Both recording-video specs updated with the code. Previous sync 2026-08-02 (arm-then-fire burst + total-anchored Stop At: specs signed off, implemented, and verified on the paired-sim rig the same day).
...
| Recording · Video (ramp) | [recording-video.svg](recording-video.svg) | `Watch/WatchControlView.swift` (`recordingScreen`; `burstToggle` — segmented, sides select states; `timedBurstRow` — armed chips 1s/4s/8s, tick = persisted default; `SlideToStop`, `intervalDots`) | ✅ |
