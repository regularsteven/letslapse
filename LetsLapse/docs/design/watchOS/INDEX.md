...
| Recording · Controls (Video) | [recording-controls.svg](recording-controls.svg) | `controlsTab`, `burstRateLadder`, `crownOwner == .burstRate` — **⚠️ found stale 2026-09-05 during the iOS Display pass, not fixed there**: `controlsTab` has carried a **Dim screen** toggle (`dimScreenRow`, `WatchControlView.swift`) since 2026-08-25 and neither controls mirror has ever drawn it. It is also about to be renamed — the iOS side moves the setting into a DISPLAY section as **Blackout viewfinder** (`iOS/settings.display.portrait.svg`); the wire command (`setDimDuringShoot`) and the state-frame key do not change, so only this row's LABEL does. Draw it with the rename, not before. | ⚠️ |
| Recording · Controls (Interval) | [recording-controls.interval.svg](recording-controls.interval.svg) | `controlsTab` interval branch — `intervalRow`, `framesRow` — **stale 2026-08-16**: while a Holy Grail or Scanner run is live this tab no longer shows Every/Blend at all. The phone refuses every setting on it mid-run (`!isCapturing`), so showing them would be showing controls that do nothing; it now carries `rampReadoutRow` (exposure + "shutter at max · ISO ramping" / "past the sensor's limit") or `deleteLastFrameRow`, under a `runModeCaption` naming the mode and its output format — **⚠️ found stale 2026-09-05 during the iOS Display pass, not fixed there**: `controlsTab` has carried a **Dim screen** toggle (`dimScreenRow`, `WatchControlView.swift`) since 2026-08-25 and neither controls mirror has ever drawn it. It is also about to be renamed — the iOS side moves the setting into a DISPLAY section as **Blackout viewfinder** (`iOS/settings.display.portrait.svg`); the wire command (`setDimDuringShoot`) and the state-frame key do not change, so only this row's LABEL does. Draw it with the rename, not before. | ⚠️ |
