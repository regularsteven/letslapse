...
| 2 | **The minimal dataset**: one `records.aar` (deterministic Apple Archive of the registry's records and sidecars), one `poster.jpg`; sources and blends stay; `originID` is the server key; the manifest cap and its overflow shape. (Until 2026-09-18 the per-project `luts/` copies went up as `lut` objects nothing downloaded — a LUT is a library asset now, `lut-library-assets.md`; the account-level surface is an open ask.) | plan §10, `PicPlaceSyncPolicy`, `PicPlaceSyncRun`, `PicPlacePoster` |
