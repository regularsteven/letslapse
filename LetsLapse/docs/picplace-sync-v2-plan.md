...
| **Authored inputs** | `luts/*` (140 files / 125 MB across 101 projects, ~0.9 MB each, mostly copies), `ref/*` | ~1 MB each | once by hash — the server's account-wide dedupe makes the LUT copies free. *Superseded 2026-09-18: a LUT is a library asset, never a project object; `luts/` is not sent (`lut-library-assets.md`, ask `lut-library-server-asks.md`)* | `ref` objects |
