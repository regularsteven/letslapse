...
extension DNGArchive {
    ...
    public enum JXLEncoder {
        ...
        public static func encode(samples16: [UInt16], width: Int, height: Int, channels: Int,
                           distance: Float, effort: Int, decodeSpeed: Int, xyb: Bool, threads: Int) throws -> Data {
            guard let encoder = JxlEncoderCreate(nil) else { throw ConversionError.encode("JxlEncoderCreate") }
            ...
            if threads > 1 {
                ...
                guard JxlEncoderSetParallelRunner(encoder, JxlThreadParallelRunner, runner) == JXL_ENC_SUCCESS else {
                    throw ConversionError.encode("JxlEncoderSetParallelRunner")
                }
            }
            ...
            guard JxlEncoderSetBasicInfo(encoder, &info) == JXL_ENC_SUCCESS else { throw ConversionError.encode("JxlEncoderSetBasicInfo") }
            ...
            guard JxlEncoderSetColorEncoding(encoder, &color) == JXL_ENC_SUCCESS else { throw ConversionError.encode("JxlEncoderSetColorEncoding") }
            ...
            guard let settings = JxlEncoderFrameSettingsCreate(encoder, nil) else { throw ConversionError.encode("JxlEncoderFrameSettingsCreate") }
            ...
            guard added == JXL_ENC_SUCCESS else { throw ConversionError.encode("JxlEncoderAddImageFrame → \(added.rawValue) (\(JxlEncoderGetError(encoder).rawValue))") }
            ...
            while true {
                ...
                guard status == JXL_ENC_SUCCESS else { throw ConversionError.encode("JxlEncoderProcessOutput → \(status.rawValue) (\(JxlEncoderGetError(encoder).rawValue))") }
                break
            }
