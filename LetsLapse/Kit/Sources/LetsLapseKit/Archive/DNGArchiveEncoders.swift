...
extension DNGArchive {
    ...
    public enum TileCodec: Equatable, Sendable {
        case none
        case lj92
        ...
        public var compression: DNGArchive.Compression {
            switch self {
    ...
    public struct EncodedTiles {
        public let tiles: [Data]
    ...
    public enum TileEncoder {

        /// 16-bit stored samples (interleaved `spp`), width × height.
        public static func encode16(_ stored: [UInt16], width: Int, height: Int, samplesPerPixel spp: Int,
                    ...
                    failures.withUnsafeMutableBufferPointer { errors in
                        DispatchQueue.concurrentPerform(iterations: across * down) { index in
                            ...
                            do {
                                ...
                                case .jpeg8, .hwjpeg:
                                    throw ConversionError.encode("8-bit codecs take 8-bit tiles")
                                }
                            } catch {
            ...
            if let failure = failures.compactMap({ $0 }).first { throw ConversionError.encode(failure) }
        ...
        public static func encode8(_ stored: [UInt8], width: Int, height: Int, tile: Int, codec: TileCodec) throws -> EncodedTiles {
            let started = ProcessInfo.processInfo.systemUptime
            switch codec {
            ...
            default:
                throw ConversionError.encode("\(codec.label) takes 16-bit tiles")
            }
        }
    ...
    public enum ImageIOJPEG {
        public static func encode(rgb8: [UInt8], width: Int, height: Int, quality: Double) throws -> Data {
            let data = Data(rgb8)
            guard let provider = CGDataProvider(data: data as CFData),
                ...
                throw ConversionError.encode("CGImage for JPEG tile")
            }
            let output = NSMutableData()
            guard let destination = CGImageDestinationCreateWithData(output, "public.jpeg" as CFString, 1, nil) else {
                throw ConversionError.encode("CGImageDestination JPEG")
            }
            CGImageDestinationAddImage(destination, image, [kCGImageDestinationLossyCompressionQuality: quality] as CFDictionary)
            guard CGImageDestinationFinalize(destination) else { throw ConversionError.encode("JPEG finalize") }
            return output as Data
        }

        /// The SOF0 sampling factors of a baseline JPEG ("1x1,1x1,1x1" = 4:4:4).
        public static func samplingFactors(_ jpeg: Data) -> String {
    ...
    public enum HardwareJPEG {
        public static func encode(rgb8: [UInt8], width: Int, height: Int, quality: Double) throws -> (Data, String) {
            var pixelBuffer: CVPixelBuffer?
            guard CVPixelBufferCreate(nil, width, height, kCVPixelFormatType_32BGRA, nil, &pixelBuffer) == kCVReturnSuccess,
                  let buffer = pixelBuffer else { throw ConversionError.encode("pixel buffer") }
            ...
            // The hardware-only keys reached iOS in 17.4; before that (and on
            // the Kit's iOS 16 floor) VideoToolbox picks the encoder itself.
            ...
            guard status == noErr, let session else { throw ConversionError.encode("VTCompressionSession(jpeg, hardware) → \(status)") }
            ...
            guard encode == noErr else { throw ConversionError.encode("EncodeFrame → \(encode)") }
            VTCompressionSessionCompleteFrames(session, untilPresentationTimeStamp: .invalid)
            guard semaphore.wait(timeout: .now() + 10) == .success else { throw ConversionError.encode("hardware JPEG timed out") }
            guard encodeStatus == noErr, !output.isEmpty else { throw ConversionError.encode("hardware JPEG status \(encodeStatus), \(output.count) bytes") }
