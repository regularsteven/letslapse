...
final class MetalDemosaic {
    enum Method: String { case mhc, bin2 }

    struct Result {
        let frame: RGBFrame
        let uploadMilliseconds: Double
        let demosaicMilliseconds: Double
        let resizeMilliseconds: Double
        let readbackMilliseconds: Double
    }
    ...
    init() throws {
        guard let device = MTLCreateSystemDefaultDevice(), let queue = device.makeCommandQueue() else {
            throw SpikeError.unsupported("no Metal device")
        }
        self.device = device
        self.queue = queue
        ...
        guard let mhcFunction = library.makeFunction(name: "demosaicMHC"),
            ...
            throw SpikeError.unsupported("demosaic kernels missing")
        }
    ...
    func run(_ mosaic: MosaicFrame, method: Method, targetPixels: Int?) throws -> Result {
        ...
        guard let input = device.makeTexture(descriptor: inputDescriptor) else { throw SpikeError.unsupported("mosaic texture") }
        ...
        guard let demosaiced = device.makeTexture(descriptor: rgbaDescriptor) else { throw SpikeError.unsupported("rgb texture") }
        ...
        guard let commandBuffer = queue.makeCommandBuffer(), let encoder = commandBuffer.makeComputeCommandEncoder() else {
            throw SpikeError.unsupported("command buffer")
        }
        ...
        if let error = commandBuffer.error { throw SpikeError.unsupported("demosaic failed: \(error)") }
        let demosaicMs = lap()
        ...
        if let targetPixels, targetPixels < outWidth * outHeight {
            ...
            guard let scaled = device.makeTexture(descriptor: scaledDescriptor) else { throw SpikeError.unsupported("scaled texture") }
            ...
            guard let resizeBuffer = queue.makeCommandBuffer() else { throw SpikeError.unsupported("resize buffer") }
            ...
            if let error = resizeBuffer.error { throw SpikeError.unsupported("resize failed: \(error)") }
            final = scaled
            resizeMs = lap()
        }
...
extension MetalDemosaic {
    ...
    func lanczos(rgba: [Float], width: Int, height: Int, targetPixels: Int) throws -> (width: Int, height: Int, rgb: [Float]) {
        ...
        guard let source = deviceHandle.makeTexture(descriptor: descriptor) else { throw SpikeError.unsupported("source texture") }
        ...
        scaledDescriptor.usage = [.shaderRead, .shaderWrite]
        scaledDescriptor.storageMode = .shared
        guard let destination = deviceHandle.makeTexture(descriptor: scaledDescriptor) else { throw SpikeError.unsupported("scaled texture") }
        ...
        guard let commandBuffer = queueHandle.makeCommandBuffer() else { throw SpikeError.unsupported("command buffer") }
