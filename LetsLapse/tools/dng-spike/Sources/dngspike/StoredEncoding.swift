...
struct StoredEncoding {
    ...
    var storedMax: Int { (1 << bitsPerSample) - 1 }
    /// The linearized domain is always 16-bit; 8-bit stores index a
    /// 256-entry table into it.
    var linearizedMax: Int { 65535 }
    var storedPedestal: Int { Int((Double(pedestal) / 65535 * Double(storedMax)).rounded()) }

    init(curve: Curve, bitsPerSample: Int = 16, pedestal: Int = 2048) {
        self.curve = curve
    ...
    var label: String { "\(curve.label)-\(bitsPerSample)b" }

    // MARK: - Levels the reader sees

    func levels(samplesPerPixel: Int) -> DNGArchive.Levels {
        switch curve {
        case .linear:
    ...
    private static var cubicInverseCache: [Double: [Float]] = [:]
    ...
    func encode16(_ linear: UnsafePointer<Float>, count: Int, into out: UnsafeMutablePointer<UInt16>) {
    ...
    func encode8(_ linear: UnsafePointer<Float>, count: Int, into out: UnsafeMutablePointer<UInt8>) {
